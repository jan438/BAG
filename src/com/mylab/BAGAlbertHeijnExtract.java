...
public class BAGAlbertHeijnExtract {

	int count;

	public void process_pands(String formatted_kml, String BAGId, String woonplaats, String oppervlakte) {
		...
		try {
			out = new PrintWriter(new BufferedWriter(new FileWriter(
					BAGHttpURLConnection.js_albertheijn_file, true)));
		...
		try {
			...
			for (int i = 0; i < placemarks.size(); i++) {
				...
				if (!placeMark.getGebruiksdoel().equalsIgnoreCase("winkelfunctie")) continue;
				String status = placeMark.getStatus();
				if (status.contains("'")) {
